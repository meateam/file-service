...
/**
 * Extracts the indixes names and values thrown by the duplicate key error.
 * @param error - the mongo error thrown.
 * @return string with the index names and values.
 */
export function getMongoErrorIndixes(error: MongoError): { indexName: string, values: string } {
  // extract the indixes names in the MongoError
  ...
  // prettify indixes names
  const re: RegExp = new RegExp('_1_', 'g');
  indexName = indexName.replace(re, ', ');

  // extract the indixes values of the error thrown
