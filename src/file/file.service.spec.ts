...
describe('File Logic', () => {
  ...
  describe('#createUpload', () => {
    ...
    it('should throw an error when {ownerID, parent, name} already exist', async () => {
      const newUpload1: IUpload = await UploadService.createUpload(
        testUpload.key,
        testUpload.bucket,
        'name1',
        USER.id,
      ...
      const newUpload2: IUpload = await UploadService.createUpload(
        KEY2,
        'BUCKET2',
        'name1',
        USER.id,
  ...
  describe('#createUpdate', () => {
    it('should not throw an error when {ownerID, parent, name} already exist', async () => {
      ...
      const newUpdate1: IUpload = await UploadService.createUpdate(
        KEY,
        bucket,
        'file.txt',
        USER.id,
      ...
      const newUpdate2: IUpload = await UploadService.createUpdate(
        KEY2,
        bucket,
        'file.txt',
        USER.id,
    ...
    it('should return a new upload', async () => {
      ...
      const newUpdate: IUpload = await UploadService.createUpdate(
        KEY,
        bucket,
        'file.txt',
        USER.id,
    ...
    it('should throw FileNotFoundError error', async () => {
      await UploadService.createUpdate(
        testUpload.key,
        testUpload.bucket,
        testUpload.name,
        USER.id,
    ...
    it('should stay same quota usage by the negative size of the upload', async () => {
      ...
      const newUpload: IUpload = await UploadService.createUpdate(
        KEY,
        bucket,
        testUpload.name,
        USER.id,
    ...
    it('should increase quota usage by the size of the upload', async () => {
      const oldQuota: IQuota = await QuotaService.getByOwnerID(USER.id);
      const newFile: IFile = await FileService.create(
        bucket,
        testUpload.name,
        USER.id,
        'text',
      ...
      const newQuota: IQuota = await QuotaService.getByOwnerID(USER.id);
      expect(newQuota.used).to.be.equal(
        oldQuota.used + newUpload.size + newFile.size
      );
    });
  });
  ...
  describe('#updateUploadID', () => {
    it('should update upload id', async () => {
      ...
      const myUpload: IUpload = await UploadService.getUploadById(
        testUpload.uploadID
      );
  ...
  describe('#deleteUpload', () => {
    it('should delete an existing upload', async () => {
      ...
      await UploadService.deleteUpload(testUpload.uploadID).should.eventually
        .not.be.rejected;

      await UploadService.getUploadById(
        testUpload.uploadID
    ...
    it('should decrease quota usage by the size of the upload', async () => {
      ...
      await UploadService.deleteUpload(newUpload.uploadID).should.eventually.not
        .be.rejected;
  ...
  describe('#createFile', () => {
    ...
    it('should throw error: parent app id is not the same', async () => {
      const parent = await FileService.create(
        bucket,
        'myFile',
        USER.id,
        'Text',
    ...
    it('should create a file at the root folder', async () => {
      ...
      const filesInRoot: IFile[] = await FileService.getFilesByFolder(
        null,
        USER.id
      );
  ...
  describe('#updateById', () => {
    it('should update a file', async () => {
      ...
      expect(changedFile.createdAt.getTime()).to.equal(
        file.createdAt.getTime()
      );
      expect(changedFile.createdAt.getTime()).to.equal(
        file.createdAt.getTime()
      );
      expect(changedFile.updatedAt.getTime()).to.be.greaterThan(
        file.updatedAt.getTime()
      );
      expect(changedFile.updatedAt.getTime()).to.be.greaterThan(
        changedFile.createdAt.getTime()
      );
    });
    ...
    it('should throw an error when changing a file to unique properties of another (trinity)', async () => {
      ...
      await FileService.updateById(file1.id, {
        name: 'file2.txt',
      }).should.eventually.be.rejectedWith(FileExistsWithSameName);
    });
  ...
  describe('#updateMany', () => {
    ...
    it('should not update folder`s parent to be its ancestors', async () => {
      ...
      const failed = await FileService.updateMany(
        [structure[0].id],
        partialFile
      );
      expect(failed).to.have.lengthOf(1);
    ...
    it('should update a folder parent', async () => {
      ...
      const folder2 = await FileService.create(
        bucket,
        'folder2',
        USER.id,
        FolderContentType,
  ...
  describe('#getByID', () => {
    it('should get an error when file does not exist', async () => {
      await FileService.getByKey(KEY).should.eventually.be.rejectedWith(
        FileNotFoundError
      );
    });
  ...
  describe('#getFilesByFolder', () => {
    it('should throw an error when not sending ownerID with null folder', async () => {
      await FileService.getFilesByFolder(
        null,
        null
      ).should.eventually.be.rejectedWith(ClientError);
    });
    ...
    it('should return all the files and folders directly under the given folder', async () => {
      ...
      const father = await FileService.create(
        bucket,
        'father',
        USER.id,
        FolderContentType,
    ...
    describe('Root Folder', () => {
      ...
      it('should get only the folders in the root folder', async () => {
        const structure: IFile[] = await generateFolderStructure();
        const folders = await FileService.getFilesByFolder(
          structure[0].id,
          null,
      ...
      it('should get all of the files in the root folder using empty json', async () => {
        const structure: IFile[] = await generateFolderStructure();
        const rootChildren = await FileService.getFilesByFolder(
          structure[0].id,
          null,
          {}
        );
  ...
  describe('#getDescendantsByFolder', () => {
    it('should return a recursive json object', async () => {
      const structure: IFile[] = await generateFolderStructure();
      const populated: ResFile[] = await FileService.getDescendantsByFolder(
        structure[0].id,
        structure[0].ownerID
      );
      ...
      for (let j = 0; j < populated.length; j++) {
        for (let i = 0; i < structure.length; i++) {
          if (
            String(structure[i].parent) === String((<ResFile>populated[j]).id)
          ) {
            expect((<ResFile>populated[j]).children).to.containSubset([
              { id: structure[i].id },
            ]);
          }
        }
    ...
    it('should return a recursive json object, only with folders', async () => {
      const structure: IFile[] = await generateFolderStructure();
      const populated = await FileService.getDescendantsByFolder(
        structure[0].id,
        structure[0].ownerID,
        { type: FolderContentType }
      );
      ...
      for (let i = 0; i < structure.length; i++) {
        if (
          String(structure[i].parent) === String(structure[0].id) &&
          structure[i].type === FolderContentType
        ) {
          expect(populated).to.containSubset([{ id: structure[i].id }]);
        }
        if (
      ...
      for (let j = 0; j < populated.length; j++) {
        for (let i = 0; i < structure.length; i++) {
          if (
            ...
            expect(populated[j].children).to.containSubset([
              { id: structure[i].id },
            ]);
          }
        }
  ...
  describe('#isOwner', () => {
    it('should throw an error if the file does not exist', async () => {
      await FileService.isOwner(
        REVERSE_KEY,
        USER.id
      ).should.eventually.rejectedWith(FileNotFoundError);
    });
  ...
  describe('#delete', () => {
    it('should delete a file', async () => {
      ...
      await FileService.getById(file.id).should.eventually.be.rejectedWith(
        FileNotFoundError
      );
    });
    ...
    it('should recursively delete a folder', async () => {
      ...
      for (let i = 0; i < structure.length; i++) {
        await FileService.getById(
          structure[i].id
        ).should.eventually.be.rejectedWith(FileNotFoundError);
        expect(deletedFiles).to.containSubset([{ id: structure[i].id }]);
      }
      await FileService.getById(father.id).should.eventually.be.rejectedWith(
        FileNotFoundError
      );
    });
  });
  ...
  describe('#QuotaService', () => {
    describe('#isAllowedToGetQuota', () => {
      it('should return allowed to a user requesting his own quota', async () => {
        const allowed: boolean = await QuotaService.isAllowedToGetQuota(
          USER.id,
          USER.id
        );
